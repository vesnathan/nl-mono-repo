...
function main() {
  ...
  const tops = collectTopLevelPackageDirs().filter(
    (p) => !protectedPackages.has(p),
  );
  if (tops.length === 0) {
    console.log(
  ...
  tops.forEach((pkgName) => {
    ...
    console.log(
      "  Local package names found:",
      localNames.length ? localNames.join(", ") : "(none)",
    );
    ...
    if (dependents.length === 0) {
      ...
      dependents.forEach((d) => {
        ...
        const deployed =
          deployInfo.existsTemplates || deployInfo.projectConfigHit;
        ...
        console.log(
          `      deployed detection: templatesExists=${deployInfo.existsTemplates}, projectConfigHit=${deployInfo.projectConfigHit}`,
        );
    ...
    console.log(
      `  Self deployed detection for ${pkgName}: templatesExists=${selfDeployInfo.existsTemplates}, projectConfigHit=${selfDeployInfo.projectConfigHit}`,
    );
  });
}
