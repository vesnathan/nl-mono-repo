...
async function getStackStatus(
  forceDeleteManager: ForceDeleteManager,
  stackName: string,
): Promise<string | null> {
...
program
  ...
  .action(async (options) => {
    try {
      ...
      logger.success(
        `🎉 Force delete completed successfully for: ${stackName}`,
      );
    } catch (error: unknown) {
      logger.error(
        `Force delete failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      process.exit(1);
    }
  });
...
program
  .command("all")
  .description(
    "Force delete all stacks for a stage (in reverse dependency order)",
  )
  .requiredOption("--stage <stage>", "Deployment stage (dev, prod, etc.)")
  .option(
    "--max-wait <minutes>",
    "Maximum wait time per stack in minutes",
    "20",
  )
  .action(async (options) => {
    try {
      ...
      logger.info(
        `🚀 Starting bulk force delete operation for stage: ${stage}`,
      );
      ...
      for (let i = 0; i < stackTypes.length; i++) {
        ...
        try {
          ...
          logger.success(
            `✅ Step ${i + 1}/${stackTypes.length} completed: ${stackName}`,
          );
        } catch (error: unknown) {
          const errorMsg =
          ...
          if (
            ...
            logger.warning(
              `⚠️  Stack ${getStackName(stackType, stage)} does not exist. Nothing to remove.`,
            );
          } else {
      ...
      logger.error(
        `❌ Force delete all failed: ${error instanceof Error ? error.message : String(error)}`,
      );
      process.exit(1);
    }
  });
...
program
  ...
  .option(
    "--type <type>",
    "Specific stack type (waf, shared, cwl), or all types if not specified",
  )
  .action(async (options) => {
    try {
      const stage = options.stage;
      const specificType = options.type
        ? parseStackType(options.type)
        : undefined;
      ...
      for (const stackType of stackTypes) {
        try {
          ...
          await forceDeleteManager.emptyStackS3Buckets(
          ...
          logger.error(
            `Failed to cleanup S3 for ${stackType}: ${error instanceof Error ? error.message : String(error)}`,
          );
    ...
    try {
      ...
      for (const stackType of stackTypes) {
        try {
          ...
          logger.error(
            `Failed to check status for ${stackType}: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
    } catch (error: unknown) {
