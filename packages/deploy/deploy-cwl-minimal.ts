  ...
  try {
    ...
    try {
      ...
      if (
        ...
        logger.warning(
          `Bucket creation error: ${(error as { message?: string }).message}`,
        );
      } else {
    ...
    try {
      ...
      webAclId =
        wafStack.Stacks?.[0]?.Outputs?.find(
      ...
      logger.warning(
        `Could not get WAF stack outputs: ${(error as Error).message}`,
      );
    ...
    try {
      ...
      kmsKeyId =
        sharedStack.Stacks?.[0]?.Outputs?.find(
      ...
      logger.warning(
        `Could not get shared stack outputs: ${(error as Error).message}`,
      );
    ...
    try {
      ...
      if (stackStatus && failedStates.includes(stackStatus)) {
        logger.warning(
          `Stack ${stackName} is in failed state: ${stackStatus}. Using force deletion to clean up...`,
        );
      ...
      if (errorMessage?.includes("does not exist")) {
        ...
        errorMessage?.includes("AlreadyExistsException") ||
        errorMessage?.includes("already exists")
      ) {
        logger.warning(
          `Stack ${stackName} already exists but in unexpected state. Using force deletion to clean up...`,
        );
    ...
    while (!stackComplete && !stackFailed && attempts < 60) {
      ...
      try {
        ...
        logger.warning(
          `Error checking stack status: ${(error as Error).message}`,
        );
        await new Promise((resolve) => setTimeout(resolve, 30000));
      }
    }
...
deployCwlStack()
  ...
  .catch((error) => {
    // This error is from the promise chain, can be Error
