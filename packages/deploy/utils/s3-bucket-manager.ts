...
export class S3BucketManager {
  ...
  async ensureBucketExists(bucketName: string): Promise<boolean> {
    try {
      ...
      try {
        ...
        if (bucketRegion !== this.region && !(this.region === 'us-east-1' && !locationResponse.LocationConstraint)) {
          logger.warning(`Bucket ${bucketName} exists but in region ${bucketRegion} instead of ${this.region}`);
          logger.warning(`Expected region ${this.region}, found ${bucketRegion}`);
      ...
      try {
        ...
        logger.success(`Successfully created and configured bucket ${bucketName}`);
        return true;
        ...
        if (createError.name === 'BucketAlreadyExists') {
          logger.error(`Bucket ${bucketName} already exists but is owned by another account`);
          return false;
        }
      ...
      logger.error(`Failed to ensure bucket ${bucketName} exists: ${error.message}`);
      return false;
    }
  }
  ...
  private async configureBucket(bucketName: string): Promise<void> {
    try {
      ...
      logger.warning(`Failed to configure bucket ${bucketName}: ${error.message}`);
  ...
  async objectExists(bucketName: string, key: string): Promise<boolean> {
    try {
      ...
      const listObjectsResult = response as { Contents?: Array<{ Key: string }> };
      
      // If we have Contents and the first item's Key matches our key, the object exists
      return !!(listObjectsResult.Contents?.length && listObjectsResult.Contents[0].Key === key);
    } catch (error: any) {
      logger.warning(`Error checking if object ${key} exists in bucket ${bucketName}: ${error.message}`);
      return false;
    }
  }
   ...
   */
  async countObjectsWithPrefix(bucketName: string, prefix: string): Promise<number> {
    try {
      const command = new ListObjectsV2Command({
        Bucket: bucketName,
        Prefix: prefix
      ...
      logger.warning(`Error counting objects with prefix ${prefix} in bucket ${bucketName}: ${error.message}`);
      return 0;
    }
  }
}
