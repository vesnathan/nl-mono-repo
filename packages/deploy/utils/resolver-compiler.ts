...
class ResolverCompiler {
  ...
  private localSavePathBase: string;
  private localSaveEnabled: boolean;
  ...
  constructor(options: ResolverCompilerOptions) {
    ...
    this.localSavePathBase = options.localSavePathBase || path.join(process.cwd(), '.compiled_resolvers');
    this.localSaveEnabled = options.localSaveEnabled === undefined ? false : options.localSaveEnabled;
  ...
  private async compileAndUploadSharedFile(): Promise<void> {
    if (!this.sharedFileName || !this.sharedFileS3Key) {
      this.logger.debug('No shared file specified, skipping shared file compilation.');
      return;
    }
    this.logger.info(`Compiling shared file: ${this.sharedFileName}`); // Essential log
    ...
    this.logger.debug(`Created package.json for ${this.sharedFileName} in ${tempCompileDir}`);

    // Install dependencies
    try {
      this.logger.info(`Installing dependencies for ${this.sharedFileName} in ${tempCompileDir}...`); // Essential log
      const yarnInstallOutput = execSync('yarn install --ignore-scripts --no-progress --non-interactive --frozen-lockfile', {
        cwd: tempCompileDir,
        stdio: this.debugMode ? 'pipe' : 'ignore', // Conditional stdio
        encoding: 'utf8'
      });
      this.logger.info(`Yarn install completed for ${this.sharedFileName}.`); // Essential log
      if (yarnInstallOutput) this.logger.debug(`Yarn install output for ${this.sharedFileName}:\\n${yarnInstallOutput}`);
    } catch (error: any) {
    ...
    this.logger.debug(`Created tsconfig.json for ${this.sharedFileName} in ${tempCompileDir}`);

    try {
      this.logger.info(`Running yarn tsc for ${this.sharedFileName} in ${tempCompileDir}`); // Essential log
      const tscOutput = execSync('yarn tsc --project tsconfig.json --listEmittedFiles', {
        cwd: tempCompileDir,
        stdio: this.debugMode ? 'pipe' : 'ignore', // Conditional stdio
        encoding: 'utf8'
      });
      this.logger.info(`TSC compilation completed for ${this.sharedFileName}.`); // Essential log
      if (tscOutput) this.logger.debug(`TSC Output for ${this.sharedFileName}:\\n${tscOutput}`);
    } catch (error: any) {
    ...
    try {
        ...
        this.logger.debug(`Successfully found compiled file: ${compiledJsPath}`);
    } catch (e) {
    ...
    this.logger.info(`Uploaded ${compiledJsFileName} to S3: s3://${this.s3BucketName}/${targetS3Key}`); // Essential log

    await fsPromises.rm(tempCompileDir, { recursive: true, force: true });
    this.logger.debug(`Cleaned up temp compile directory: ${tempCompileDir}`);
  }

  public async compileAndUploadResolvers(): Promise<void> {
    this.logger.info('Starting resolver compilation and upload...'); // Essential log
    await this.setupBuildDirectory();
    await this.compileAndUploadSharedFile();

    const totalFiles = this.resolverFiles.length;
    this.logger.info(`Processing ${totalFiles} resolver files...`); // Essential log

    for (let index = 0; index < totalFiles; index++) {
      const resolverFileRelativePath = this.resolverFiles[index];

      if (resolverFileRelativePath === this.sharedFileName) {
        this.logger.debug(`Skipping inlining/recompilation of shared file: ${resolverFileRelativePath} in main loop.`);
        continue;
      }

      this.logger.info(`[${index + 1}/${totalFiles}] Processing resolver: ${resolverFileRelativePath}...`); // Essential log
      ...
      try {
        ...
        this.logger.info(`Uploaded ${resolverFileRelativePath} to S3: s3://${this.s3BucketName}/${s3Key}`); // Essential log
  ...
  private async saveCompiledFileLocally(localPath: string, content: string): Promise<void> {
    try {
      ...
      this.logger.debug(`Saved compiled file to ${localPath}`);
    } catch (error) {
  ...
  private async setupBuildDirectory(): Promise<void> {
    this.logger.debug(`Setting up build directory: ${this.buildDir}`);
    ...
    try {
      ...
      this.logger.debug(`Copied ${this.gqlTypesSourceFileName} to build directory for resolver compilation context.`);
    } catch (error) {
    ...
    try {
      ...
      this.logger.debug(`Copied @aws-appsync/utils to ${utilsDest} for compilation.`);
    } catch (error) {
  ...
  private async setupBuildConfiguration(): Promise<void> { // This configures this.tempBuildDir
    ...
    this.logger.debug('Created package.json and tsconfig.json in temp build directory');
  }
  ...
  private async compileTypeScript(originalResolverSourceCode: string, resolverAbsolutePath: string): Promise<string> {
    ...
    this.logger.debug(`Starting compilation for: ${resolverFileName} (from ${resolverAbsolutePath}) using temp build directory: ${this.tempBuildDir}`);
    ...
    if (gqlTypesImportRegex.test(originalResolverSourceCode)) {
      this.logger.debug(`Found import from 'gqlTypes' in ${resolverFileName}. Inlining content.`);
      ...
      this.logger.debug(`Inlined gqlTypes.ts content into ${resolverFileName}.`);
    } else {
      this.logger.debug(`No import from 'gqlTypes' found in ${resolverFileName}. Compiling as is.`);
    }
    ...
    this.logger.debug(`Wrote (potentially modified) ${resolverFileName} to ${tempResolverPath}`);
    ...
    try {
      ...
      this.logger.debug(`Copied ${this.gqlTypesSourceFileName} to ${gqlTypesDestInTemp} (for tsconfig paths and fallback if inlining was partial)`);
    } catch (e: any) {
      this.logger.debug(`Could not copy ${this.gqlTypesSourceFileName} to temp dir (warning level originally): ${e.message}.`);
    }

    await this.setupBuildConfiguration(); 

    try {
      this.logger.info(`Installing dependencies in ${this.tempBuildDir} for ${resolverFileName}...`); // Essential log
      const yarnOutput = execSync('yarn install --ignore-scripts --frozen-lockfile --no-progress --non-interactive', { cwd: this.tempBuildDir, stdio: this.debugMode ? 'pipe' : 'ignore', encoding: 'utf8' });
      this.logger.info(`Yarn install completed for ${resolverFileName}.`); // Essential log
      if (yarnOutput) {
          this.logger.debug(`Yarn install output for ${resolverFileName}:\\n${yarnOutput}`);
      }
    } catch (error: any) {
        this.logger.error(`Yarn install failed for ${resolverFileName} in ${this.tempBuildDir}: ${error.message}`);
        if (this.debugMode && error.stdout) this.logger.error(`Yarn stdout:\\n${error.stdout.toString()}`);
        if (this.debugMode && error.stderr) this.logger.error(`Yarn stderr:\\n${error.stderr.toString()}`);
        throw error;
    }
      
    if (this.debugMode) {
      this.logger.debug(`Contents of ${this.tempBuildDir} before esbuild for ${resolverFileName}:`);
      this.logger.debug(execSync('ls -R', { cwd: this.tempBuildDir, encoding: 'utf8' }).toString());
    }
    ...
    try {
      this.logger.info(`Running esbuild for ${resolverFileName}...`); // Essential log
      await esbuild.build(esbuildConfig);
      this.logger.info(`esbuild completed for ${resolverFileName}.`); // Essential log

      const compiledJsPath = esbuildConfig.outfile;
      if (!compiledJsPath) { // Add this check
        this.logger.error(`esbuildConfig.outfile is undefined after build for ${resolverFileName}. This should not happen if 'write: true'.`);
      ...
      try {
        ...
        this.logger.error(`Compiled file not found at ${compiledJsPath} after esbuild.`);
      ...
      jsContentWithHeader = this.addHeaderToJs(jsContentWithHeader, resolverAbsolutePath, s3Key);

      this.logger.debug(`Successfully read compiled JS for ${resolverFileName}`);
      return jsContentWithHeader;
    } catch (error: any) {
      this.logger.error(`TypeScript compilation failed for ${resolverAbsolutePath}: ${error.message}`);
      if (this.debugMode && error.stdout) this.logger.error(`Compilation stdout:\\n${error.stdout.toString()}`);
      if (this.debugMode && error.stderr) this.logger.error(`Compilation stderr:\\n${error.stderr.toString()}`);
      // Log contents of tempBuildDir for debugging if in debug mode
      if (this.debugMode) {
        try {
          this.logger.error(`Contents of ${this.tempBuildDir} on failure for ${resolverFileName}:`);
          this.logger.error(execSync('ls -R', { cwd: this.tempBuildDir, encoding: 'utf8' }).toString());
        } catch (lsError: any) {
          this.logger.error(`Could not list contents of ${this.tempBuildDir}: ${lsError.message}`);
        }
      }
      throw error;
    } finally {
      if (!this.debugMode) {
        try {
          ...
          this.logger.warning(`Failed to clean up temp build directory ${this.tempBuildDir} for ${resolverFileName}: ${cleanupError.message}`);
        }
      } else {
        this.logger.debug(`Debug mode: Preserving temp build directory: ${this.tempBuildDir} for ${resolverFileName}`);
      }
    }
  }

  // This method seems to be a duplicate or an alternative way of handling uploads.
  // It was causing issues in the previous diff. I'll comment it out for now.
  // If it's needed, it should be reviewed and integrated properly.
  /*
  private async uploadCompiledResolvers(
    ...
    try {
        ...
        if (bucketError.name === 'NoSuchBucket' || bucketError.name === 'NotFound') {
            ...
            try {
              const createParams: any = { // Use any for createParams to handle conditional property
                Bucket: bucketName,
              };
              ...
              try {
                 ...
                 this.logger.error(`Bucket ${bucketName} was created but is not accessible: ${verifyError.message}`);
                ...
                throw createError; // Re-throw if bucket creation fails
            }
        } else if (bucketError.Code === 'AccessDenied') {
             this.logger.error(`Access Denied to bucket ${bucketName}. Please check permissions.`);
             throw bucketError;
        } else {
            this.logger.warning(`Could not verify bucket ${bucketName} (may be ok if it exists and is owned by you): ${bucketError.message}`);
            // If it's not a "NotFound" or "NoSuchBucket" error, assume it exists but we might not have HeadBucket perms.
    ...
    for (const [resolverName, info] of resolverMap) {
        try {
            ...
            await this.uploadToS3(info.s3Key, jsContent, 'application/javascript');
  ...
  */
  ...
  private async findJavaScriptFileByName(jsFileName: string): Promise<string[]> {
    ...
    this.logger.warning(`findJavaScriptFileByName is called for ${jsFileName}, but its usage is currently under review.`);
    return [];
    /*
