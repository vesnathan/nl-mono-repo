 ...
 */
export async function addAppSyncBucketPolicy(bucketName: string, region: string): Promise<void> {
  ...
  const retryOperation = async <T>(operation: () => Promise<T>): Promise<T> => {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        lastError = error;
        logger.warning(`Operation failed (attempt ${attempt}/${MAX_RETRIES}): ${error.message}`);
  ...
  try {
    ...
    await retryOperation(async () => {
      try {
        ...
        logger.error(`Bucket ${bucketName} does not exist or is not accessible: ${error.message}`);
        throw new Error(`Cannot add policy to non-existent bucket ${bucketName}`);
      }
    });
    ...
    try {
      await retryOperation(async () => {
        try {
          ...
          logger.info(`No existing bucket policy found, will create new policy`);
        }
      });
    } catch (error: any) {
      logger.warning(`Could not retrieve bucket policy after multiple attempts: ${error.message}`);
    ...
    if (!existingPolicy.Statement.some((stmt: any) => 
    ...
    if (policyUpdated) {
      await retryOperation(async () => {
        try {
          ...
          logger.success(`Updated bucket policy for ${bucketName} to allow AppSync access`);
        } catch (error: any) {
    ...
    await retryOperation(async () => {
      try {
        ...
        logger.success(`Successfully verified bucket ${bucketName} is accessible after policy update`);
      } catch (error: any) {
        logger.warning(`Bucket ${bucketName} may have access issues after policy update: ${error.message}`);
        throw error;
      }
    });
  } catch (error: any) {
    logger.error(`Failed to configure bucket policy for AppSync access: ${error.message}`);
    throw error;
  }
}
 ...
 * @param region AWS region
 */
export async function verifyResolversAccessible(bucketName: string, stage: string, region: string): Promise<boolean> {
  const s3Client = new S3Client({ region });
  
  try {
    logger.info(`Verifying resolver files are accessible in bucket ${bucketName}...`);
    ...
    try {
      ...
      const listOperation = async () => {
        try {
          ...
          logger.warning(`Error listing objects, retrying... (${error.message})`);
          throw error;
        }
      };
      ...
      const listObjectsResult = listResponse as { Contents?: Array<{ Key: string }> };
      const resolverCount = listObjectsResult.Contents?.length || 0;
      
      if (resolverCount === 0) {
        logger.warning(`No resolver files found in bucket ${bucketName} with prefix ${resolversPrefix}`);
        return false;
      }
          ...
          if (firstResolver) {
            ...
            const testOperation = async () => {
              try {
                ...
                logger.warning(`Error getting resolver object, retrying... (${error.message})`);
                throw error;
              }
            };
            ...
            while (retryCount < maxRetries && !accessSuccessful) {
              try {
                ...
                logger.success(`Successfully verified bucket ${bucketName} is readable`);
              } catch (error: any) {
                retryCount++;
                if (retryCount >= maxRetries) {
                  logger.warning(`Failed to verify resolver access after multiple retries: ${error.message}`);
                  break;
                }
                // Exponential backoff
                await new Promise(resolve => setTimeout(resolve, 1000 * Math.pow(2, retryCount)));
            ...
            if (!accessSuccessful) {
              logger.warning(`Could not verify read access to resolver ${firstResolver}`);
              return false;
            }
          }
        } catch (error: any) {
          logger.warning(`Bucket ${bucketName} may have access issues: ${error.message}`);
          // Continue, don't fail on access test
        }
      }
      
      logger.success(`Found ${resolverCount} resolver files in bucket ${bucketName}`);
