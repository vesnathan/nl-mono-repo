#!/usr/bin/env ts-node
/**
 * App Builder Studio Quick Deploy Script
 *
 * Non-interactive deployment script with pre-configured settings for App Builder Studio dev environment.
 *
 * Usage:
 *   yarn deploy:abs:dev
 *
 * Or for update strategy:
 *   yarn deploy:abs:dev:update
 *
 * Or for production:
 *   yarn deploy:abs:dev --stage prod
 */

import { config } from "dotenv";
import * as path from "path";
import { deployAppBuilderStudio } from "./packages/app-builder-studio/deploy";
...
async function main() {
  ...
  const logFile = path.join(
    logDir,
    `app-builder-studio-deploy-${options.stage}-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
  );
  setLogFile(logFile);

  logger.info("=".repeat(80));
  logger.info("App Builder Studio Quick Deployment");
  logger.info("=".repeat(80));
  ...
  const stackName = getStackName(StackType.AppBuilderStudio, options.stage);
  ...
  try {
    ...
    if (options.strategy === "replace") {
      ...
      if (stackExists) {
        ...
        const buckets = [
          `nlmonorepo-appbuilderstudio-templates-${options.stage}`,
          `nlmonorepo-appbuilderstudio-frontend-${options.stage}`,
        ];
    ...
    logger.info("\nStarting deployment...");
    await deployAppBuilderStudio(deployOptions);
