...
async function removeWafStack() {
  ...
  try {
    ...
    logger.info(
      "Wait for the stack to be deleted in the AWS CloudFormation console",
    );
    logger.info(`Region: ${WAF_REGION}`);
  } catch (error: unknown) {
    logger.error(
      `Failed to delete WAF stack ${stackName}: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(1);
  }
}
