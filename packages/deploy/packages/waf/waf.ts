...
async function uploadWafTemplates(
  s3Client: S3Client,
  bucketName: string,
): Promise<void> {
  const templatesPath = TEMPLATE_RESOURCES_PATHS[StackType.WAF];
  logger.debug(
    `Uploading WAF templates from ${templatesPath} to bucket ${bucketName}...`,
  );

  try {
    const files = await glob(
      path.join(templatesPath, "**/*.yaml").replace(/\\/g, "/"),
    );
    if (files.length === 0) {
      logger.warning(
        `No YAML templates found in ${templatesPath}. Ensure templates are present.`,
      );
      return;
    }
...
export async function deployWaf(options: DeploymentOptions): Promise<void> {
  ...
  const stopSpinner = logger.infoWithSpinner(
    `Starting WAF stack deployment in ${WAF_REGION}`,
  );
  ...
  try {
    ...
    try {
      await s3Client.send(
        new CreateBucketCommand({ Bucket: templateBucketName }),
      );
      logger.success(
        `Successfully created or verified bucket: ${templateBucketName}`,
      );
    } catch (error) {
      if (
        (error as Error).name === "BucketAlreadyOwnedByYou" ||
        (error as Error).name === "BucketAlreadyExists"
      ) {
        logger.debug(`Bucket ${templateBucketName} already exists.`);
      } else {
        logger.error(
          `Failed to create bucket ${templateBucketName}: ${(error as Error).message}`,
        );
        throw error;
      }
    }
    ...
    if (stackExists) {
      ...
      try {
        ...
        await waitUntilStackUpdateComplete(
          { client: cfClient, maxWaitTime: 3600 },
          { StackName: stackName },
        );
