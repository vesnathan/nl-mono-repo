    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Continue without throwing to keep deployment successful even if seeding fails
        }
      } catch (seedError) {
        // Post-deploy orchestration errors should be surfaced, but seeding failures are non-fatal
        logger.error(
          `CWL post-deploy tasks encountered an error: ${seedError instanceof Error ? seedError.message : seedError}`,
        );
