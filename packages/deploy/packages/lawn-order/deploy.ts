...
export async function deployLawnOrder(
  ...
  const templateBucketName = `nlmonorepo-lawnorder-templates-${stage}`;
  const stackName = `nlmonorepo-lawnorder-${stage}`;
  ...
  const templatesDir = path.resolve(__dirname, "../../templates/lawn-order");
  ...
  const parameters = [
    ...
    { ParameterKey: "AppName", ParameterValue: "lawnorder" },
