...
async function retryOperation<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.warning(
        `Operation failed (attempt ${attempt}/${maxRetries}): ${error instanceof Error ? error.message : String(error)}`,
      );
...
export async function deployShared(options: DeploymentOptions): Promise<void> {
  ...
  const stopSpinner = logger.infoWithSpinner(
    `Starting Shared stack deployment in ${region}`,
  );
  ...
  try {
    ...
    try {
      logger.debug(
        `Checking if templates bucket exists: ${templateBucketName} in region ${region}`,
      );
      await s3.headBucket({ Bucket: templateBucketName });
      logger.debug(`Templates bucket ${templateBucketName} exists`);
    } catch (error: unknown) {
      logger.debug(
        `Creating templates bucket: ${templateBucketName} in region ${region}`,
      );
      logger.debug(
    ...
    try {
      logger.debug(
        `Ensuring CloudFormation role has S3 access to templates bucket...`,
      );
      ...
      try {
        ...
        logger.info(
          `Could not update role policy (role may not exist yet): ${roleError instanceof Error ? roleError.message : String(roleError)}`,
        );
      }
    } catch (error: unknown) {
      logger.warning(
        `Failed to configure IAM role S3 access: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    ...
    logger.debug(
      `Searching for templates with pattern: **/*.yaml in ${TEMPLATE_RESOURCES_PATHS[StackType.Shared]}`,
    );
    ...
    try {
      ...
      const templateFiles = await findYamlFiles(
        TEMPLATE_RESOURCES_PATHS[StackType.Shared],
      );

      logger.debug(
        `Found ${templateFiles.length} template files in ${TEMPLATE_RESOURCES_PATHS[StackType.Shared]}`,
      );
      logger.debug(`Template files: ${JSON.stringify(templateFiles, null, 2)}`);

      if (templateFiles.length === 0) {
        logger.error(
          "No template files found for shared resources. Check TEMPLATE_RESOURCES_PATHS.shared and file permissions.",
        );
        throw new Error("No template files found for shared resources.");
      }

      for (const file of templateFiles) {
        const s3Key = relative(
          TEMPLATE_RESOURCES_PATHS[StackType.Shared],
          file,
        ).replace(/\\/g, "/");

        logger.debug(
          `Uploading ${file} to s3://${templateBucketName}/${s3Key}`,
        );
    ...
    try {
      ...
      if (stackStatus === "ROLLBACK_COMPLETE") {
        logger.warning(
          `Stack ${stackName} is in ROLLBACK_COMPLETE state. Deleting and recreating...`,
        );
