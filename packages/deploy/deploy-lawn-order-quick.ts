#!/usr/bin/env ts-node
/**
 * Lawn Order Quick Deploy Script
 *
 * Non-interactive deployment script with pre-configured settings for Lawn Order dev environment.
 *
 * Usage:
 *   yarn deploy:lawn-order:dev
 *
 * Or for update strategy:
 *   yarn deploy:lawn-order:dev:update
 *
 * Or for production:
 *   yarn deploy:lawn-order:dev --stage prod
 */
...
async function main() {
  ...
  const logFile = path.join(
    logDir,
    `lawn-order-deploy-${options.stage}-${new Date().toISOString().replace(/[:.]/g, "-")}.log`,
  );
  setLogFile(logFile);

  logger.info("=".repeat(80));
  logger.info("Lawn Order Quick Deployment");
  logger.info("=".repeat(80));
  ...
  const stackName = getStackName(StackType.LawnOrder, options.stage);
  ...
  try {
    ...
    logger.info("\nStarting deployment...");
    await deployLawnOrder(deployOptions);
