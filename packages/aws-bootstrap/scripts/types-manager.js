...
function regenerateTypesFile(typesPath, stackTypes) {
  ...
  const beforeEnum = content.substring(
    0,
    content.indexOf("export enum StackType"),
  );
  const afterResources = content.substring(
    content.indexOf("export const getStackName"),
  );
  ...
  const templateLines = stackTypes
    .map((st) => {
  ...
  const resourcesLines = stackTypes
    .map((st) => {
      const templateDir = stackTypeToTemplateDir(st);
