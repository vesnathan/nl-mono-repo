...
async function removePackageInteractive() {
  ...
  async function listPackagesForRemoval() {
    ...
    const protected = new Set(["aws-bootstrap", "deploy", "shared", "waf"]);
    return items.filter((i) => !protected.has(i));
  }
...
async function main() {
  // If script called with 'delete' argument, run interactive removal flow
  const rawArg = process.argv[2] && process.argv[2].toLowerCase();
