...
async function deploymentMenu() {
    ...
    const { instanceType } = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          {
            name: "t3.small - ~$15.00/month - Light development",
            value: "t3.small",
          },
          {
            name: "t3.medium - ~$30.00/month - Full development",
            value: "t3.medium",
          },
        ],
        default: "t3.micro",
  ...
  if (action === "deploy") {
    ...
    if (costOptimizations.instanceType) {
      logger.info(
        `Instance: ${costOptimizations.instanceType}, Auto-Stop: ${costOptimizations.autoStop}, HTTPS: always enabled`,
      );
    ...
    logger.info(
      `Updating existing stack for stage: ${stage} with cost optimizations`,
    );
    if (costOptimizations.instanceType) {
      logger.info(
        `Instance: ${costOptimizations.instanceType}, Auto-Stop: ${costOptimizations.autoStop}, HTTPS: always enabled`,
