...
export class DeploymentManager {
  ...
  async deploy(options: DeploymentOptions): Promise<void> {
    logger.info(
      `Deploying VSCode server with stack name: ${options.stackName}`,
    );
    ...
    if (stackExists) {
      ...
      if (!update) {
        ...
        logger.info(
          "Waiting for stack to be fully deleted before redeploying...",
        );
        let deleted = false;
        for (let i = 0; i < 60; i++) {
          // up to 10 minutes
        ...
        if (!deleted) {
          logger.error(
            "Timed out waiting for stack deletion. Please check the AWS Console.",
          );
          return;
        }
    ...
    const parameters: Parameter[] = [
      ...
      {
        ParameterKey: "AutoStopEnabled",
        ParameterValue: process.env.COST_OPT_AUTO_STOP || "true",
      },
    ];
  ...
  private async printStackFailureReasons(stackName: string): Promise<void> {
    try {
      ...
      if (failedEvents && failedEvents.length > 0) {
        ...
        logger.error(
          "No CloudFormation failure events found. Check the AWS Console for more details.",
        );
      }
    } catch (err: any) {
      logger.error(
        "Could not fetch stack failure details: " + (err?.message || err),
      );
    }
  }
  ...
  private async waitForStackCompletion(
    stackName: string,
    spinner: ora.Ora,
  ): Promise<void> {
  ...
  private async displayStackOutputsWithInstallationTracking(
    stackName: string,
  ): Promise<void> {
    try {
      const command = new DescribeStacksCommand({
      ...
      const instanceId = stack.Outputs.find(
        (o) => o.OutputKey === "InstanceId",
      )?.OutputValue;
      const publicIp = stack.Outputs.find(
        (o) => o.OutputKey === "PublicIP",
      )?.OutputValue;
      const vscodeUrl = stack.Outputs.find(
        (o) => o.OutputKey === "VSCodeURL",
      )?.OutputValue;
      const sshCommand = stack.Outputs.find(
        (o) => o.OutputKey === "SSHCommand",
      )?.OutputValue;
      ...
      logger.info("\n💡 Important Notes:");
      logger.info(
        "• HTTPS uses a self-signed certificate - accept the browser security warning",
      );
      logger.info(
        "• The instance will auto-stop after 2 hours of inactivity (if enabled)",
      );
  ...
  private async monitorInstallationProgressRealTime(
    publicIp: string,
  ): Promise<void> {
    ...
    while (
      ...
      Date.now() - startTime < maxWaitTime * 1000
    ) {
      try {
        ...
        const getLog = spawn("ssh", [
          ...
          "-o",
          "LogLevel=ERROR", // Suppress SSH verbose output
          `ec2-user@${publicIp}`,
        ...
        if (
          logOutput &&
          !logOutput.includes("LOG_NOT_READY") &&
          logOutput.trim() !== ""
        ) {
          ...
          const relevantLines = lines.filter((line) =>
            this.shouldDisplayLogLine(line),
          );
          ...
          if (
            logOutput.includes("✅ Installation completed successfully") ||
          ...
          const recentFailures = recentLines.filter((l) =>
            l.includes("❌ Essential package installation failed"),
          ).length;
          const recentSuccesses = recentLines.filter(
            (l) => l.includes("✅") || l.includes("✓"),
          ).length;

          if (recentFailures >= 3 && recentSuccesses === 0) {
            logger.error(
              "❌ Installation appears to have failed. Multiple recent package installation failures detected.",
            );
            logger.info("💡 You can check the full log with:");
            logger.info(
              `   ssh -i ec2-vscode-server-dev-key.pem ec2-user@${publicIp} "sudo tail -f /var/log/code-server-install.log"`,
            );
            break;
          }
          ...
          if (consecutiveFailures === 0) {
            logger.info(
              "   ⏳ Instance starting up, waiting for installation logs...",
            );
          }
          consecutiveFailures++;
        } else {
    ...
    if (!installationComplete) {
      ...
      logger.info(
        "💡 The installation may still be running. You can check with:",
      );
      logger.info(
        `   ssh -i ec2-vscode-server-dev-key.pem ec2-user@${publicIp} "sudo tail -f /var/log/code-server-install.log"`,
      );
  ...
  private async displayStackOutputsQuick(stackName: string): Promise<void> {
    try {
      ...
      logger.info("\n💡 Important Notes:");
      logger.info(
        "• The server is installing packages - it may take 5-10 minutes to be fully ready",
      );
      logger.info(
        "• HTTPS uses a self-signed certificate - accept the browser security warning",
      );
  ...
  private async displayStackOutputs(stackName: string): Promise<void> {
    try {
      ...
      logger.info(
        "• Restore workspace: ~/restore-workspace.sh <backup-filename>",
      );
  ...
  private async waitForInstanceReady(stack: Stack): Promise<void> {
    try {
      ...
      if (!instanceRunning) {
        logger.warning(
          "⚠️ Instance did not reach running state within 5 minutes",
        );
        return;
      }
  ...
  private async monitorInstallationProgress(
    ...
    const installationSteps = [
      {
        name: "System Updates",
        marker: "Starting cost-optimized code-server installation",
        timeout: 300,
      },
      {
        name: "Essential Package Downloads",
        marker: "Step 2/8: Installing essential system packages",
        timeout: 60,
      },
      {
        name: "curl Installation",
        marker: "Installing curl (web client for downloads)",
        timeout: 60,
      },
      {
        name: "git Installation",
        marker: "Installing git (version control system)",
        timeout: 60,
      },
      {
        name: "Archive Utilities Installation",
        marker: "Installing compression utilities",
        timeout: 60,
      },
      {
        name: "System Utilities Installation",
        marker: "Installing system utilities",
        timeout: 60,
      },
      {
        name: "Development Tools Installation",
        marker: "Installing additional development tools",
        timeout: 60,
      },
      {
        name: "Package Verification",
        marker: "All essential packages verified successfully",
        timeout: 60,
      },
      {
        name: "AWS CLI Installation",
        marker: "Step 3/8: Installing AWS CLI v2",
        timeout: 240,
      },
      {
        name: "NVM Installation",
        marker: "Step 4/8: Installing NVM",
        timeout: 180,
      },
      {
        name: "Node.js & npm Installation",
        marker: "Step 5/8: Installing Node.js via NVM",
        timeout: 240,
      },
      {
        name: "Yarn Installation",
        marker: "Step 6/8: Installing Yarn package manager",
        timeout: 120,
      },
      {
        name: "Development Environment Setup",
        marker: "Step 7/8: Setting up development environment",
        timeout: 180,
      },
      {
        name: "VS Code Server Installation",
        marker: "Step 8/8: Installing VS Code Server",
        timeout: 300,
      },
      {
        name: "Package Installation Summary",
        marker: "All package installations completed successfully",
        timeout: 60,
      },
      {
        name: "SSL Certificate Generation",
        marker: "Generating self-signed SSL certificate",
        timeout: 120,
      },
      {
        name: "Service Configuration",
        marker: "Starting code-server service",
        timeout: 120,
      },
      {
        name: "Final Verification",
        marker: "SUCCESS: code-server service started",
        timeout: 60,
      },
    ];
      ...
      try {
        ...
        if (logOutput.includes(step.marker)) {
          ...
          if (step.name === "VS Code Server Installation") {
            ...
            logger.info(
              "📋 All development tools and packages have been installed",
            );
          }
        } else if (
          ...
          logger.info(
            `⏳ ${step.name} in progress... (${Math.floor(elapsed)}s elapsed)`,
          );
        }
      } catch (error: any) {
  ...
  private async checkCodeServerInstallation(publicIp: string): Promise<void> {
    try {
      ...
      logger.warning(
        `Could not verify code-server installation: ${error.message}`,
      );
    }
  }
  ...
  private async diagnoseInstallationIssues(
    publicIp: string,
    logOutput: string,
  ): Promise<void> {
  ...
  private async checkVSCodeServerReady(vscodeUrl: string): Promise<void> {
    ...
    while (!vscodeReady && attempts < maxAttempts) {
      try {
        ...
        execSync(`curl -s --max-time 5 --head "${vscodeUrl}" > /dev/null`, {
          stdio: "ignore",
        });
        vscodeReady = true;
        ...
        logger.info(
          `🔄 VS Code server connectivity check ${attempts + 1}/${maxAttempts}...`,
        );
        await new Promise((resolve) => setTimeout(resolve, 10000)); // Wait 10 seconds
        attempts++;
      }
    ...
    if (!vscodeReady) {
      ...
      logger.info(
        "💡 The server may still be starting up. Try accessing it in a few minutes.",
      );
      logger.info(
        '💡 You can check the installation log with: yarn deployment-menu -> "Tail code-server install log"',
      );
    }
  }
}
