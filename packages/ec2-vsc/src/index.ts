...
async function main() {
  async function ensureKeyPair(keyName: string) {
    ...
    try {
      ...
      if (e.name === "InvalidKeyPair.NotFound") {
        logger.debug(
          `Key pair ${keyName} does not exist in AWS - ready to create new one.`,
        );
      } else {
        logger.warning(
          `Could not check/delete existing key pair: ${e.message}`,
        );
      }
    }
  ...
  program
    ...
    .action(async (options) => {
      ...
      if (!keyExists) {
        logger.warning(
          `Key file '${keyFile}' not found. SSH will be disabled.`,
        );
      ...
      const choices = [
        {
          name: `SSH into instance (${publicIp})`,
          value: "ssh",
          disabled: !keyExists ? "Key file not found" : false,
        },
        {
          name: `Open VSCode server in browser (${vscodeUrl})`,
          value: "vscode",
        },
      ...
      while (true) {
        ...
        if (action === "ssh") {
          if (!keyExists) {
            ...
            logger.info(
              `If connection hangs, check: 1) Security Group allows SSH from your IP, 2) Instance is running`,
            );
            ...
            const timeout = setTimeout(() => {
              logger.warning(
                "SSH connection is taking longer than expected...",
              );
          ...
          logger.info(
            `ssh -i ${keyFile} -o ConnectTimeout=30 -o StrictHostKeyChecking=no ec2-user@${publicIp} 'tail -n 50 /var/log/code-server-install.log'`,
          );
          ...
          try {
            ...
            if (instanceId) {
              ...
              if (instance) {
                ...
                logger.info(
                  `Instance Status: ${instance.StateReason?.Message || "N/A"}`,
                );
  ...
  program
    ...
    .action(async (options) => {
      try {
        logger.warning(
          "This will permanently delete the CloudFormation stack and all associated resources!",
        );
        if (!options.force) {
          const answers = await inquirer.prompt([
            {
        ...
        logger.success(
          "DeleteStack command sent. Monitoring stack deletion...",
        );

        // Wait for stack deletion to complete
        const { DescribeStacksCommand } = await import(
          "@aws-sdk/client-cloudformation"
        );
        ...
        while (!stackDeleted) {
          try {
            ...
            if (stack.StackStatus === "DELETE_COMPLETE") {
              ...
              stack.StackStatus &&
              stack.StackStatus.endsWith("FAILED")
            ) {
              logger.error(
                `Stack deletion failed with status: ${stack.StackStatus}`,
              );
              process.exit(1);
            }
          } catch (err: any) {
