...
AWS_ACCOUNT_ID=${answers.accountId}`;
      try {
        ...
        logger.warning(
          `Could not save credentials to deploy/.env: ${deployErr.message}`,
        );
        if (deployErr.code === "ENOENT") {
          logger.warning(
            "The directory for deploy/.env does not exist. Please create the deploy package or its .env file manually.",
          );
        }
      }
      ...
      try {
        ...
        logger.warning(
          `Could not save credentials to local .env: ${localErr.message}`,
        );
      }
    } catch (error) {
      logger.warning(
        "Could not save credentials to any .env file. They will only persist for this session.",
      );
    }
...
async function loadCredentialsFromDeployPackage(): Promise<void> {
  try {
    ...
    logger.debug(
      "No existing AWS credentials found, will prompt for new credentials",
    );
  } catch (error: any) {
