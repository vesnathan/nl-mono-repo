...
TABLE_NAME=$(node -e "const fs=require('fs'); const path=require('path'); const stage=process.argv[1]; try{ const p=path.join(__dirname,'../../../../deploy/deployment-outputs.json'); const content=fs.readFileSync(p,'utf8'); const obj=JSON.parse(content); const stack=obj.stacks && obj.stacks['TheStoryHub']; if(stack && stack.outputs){ for(const out of stack.outputs){ const en=(out.ExportName||'').toLowerCase(); const ok=(out.OutputKey||'').toLowerCase(); if(en.includes('datatable')||ok.includes('datatable')||ok.includes('datatablename')){ console.log(out.OutputValue); process.exit(0);} } } process.exit(1);}catch(e){ process.exit(1);}" "$STAGE" 2>/dev/null || true)
...
if [ -z "$TABLE_NAME" ]; then
    ...
    TABLE_NAME=$(aws cloudformation list-exports \
        ...
        --output text 2>/dev/null || echo "nlmonorepo-tsh-datatable-${STAGE}")
fi
...
ls -al packages/the-story-hub/backend/scripts

# Use yarn to run the seeding script with local dependencies
if command -v yarn &> /dev/null; then
    echo "[DEBUG] Running: yarn --cwd packages/the-story-hub/backend run seed"
    if yarn --cwd packages/the-story-hub/backend run seed; then
        echo "[DEBUG] yarn --cwd packages/the-story-hub/backend run seed succeeded"
    else
        echo "[DEBUG] yarn --cwd failed, attempting yarn workspace fallback"
        # Try workspace command (works if workspace name is correct)
        if yarn workspace tshbackend run seed; then
            echo "[DEBUG] yarn workspace tshbackend run seed succeeded"
        else
...
if [ -n "$TABLE_NAME" ]; then
    ...
    if command -v aws &> /dev/null && aws --version &> /dev/null; then
        TABLE_NAME=$(aws cloudformation list-exports \
            --region "$AWS_REGION" \
            --query "Exports[?Name=='tsh-DataTableName-${STAGE}'].Value" \
            --output text 2>/dev/null || echo "nlmonorepo-tsh-datatable-${STAGE}")
    else
        # AWS CLI not available, use default naming convention
        TABLE_NAME="nlmonorepo-tsh-datatable-${STAGE}"
...
if command -v yarn &> /dev/null; then
    echo "[DEBUG] Running: yarn --cwd packages/the-story-hub/backend run seed:db"
    if yarn --cwd packages/the-story-hub/backend run seed:db; then
        echo "[DEBUG] Seeding succeeded"
    else
        echo "[DEBUG] yarn --cwd failed, attempting yarn workspace fallback"
        if yarn workspace tshbackend run seed:db; then
            echo "[DEBUG] yarn workspace tshbackend run seed:db succeeded"
        else
