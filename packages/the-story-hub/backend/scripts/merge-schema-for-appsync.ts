...
files.forEach(file => {
  ...
  for (const line of lines) {
    ...
    // Check if we're entering the base type definition (skip these)
    if (line.trim() === 'type Query {' ||
    ...
    if (inQuery || inMutation || inSubscription) {
      ...
      // Skip only the initial empty lines before we start
...
console.log(`   Query fields: ${queryFields.filter(l => l.trim() && !l.trim().startsWith('@')).length}`);
console.log(`   Mutation fields: ${mutationFields.filter(l => l.trim() && !l.trim().startsWith('@')).length}`);
console.log(`   Subscription fields: ${subscriptionFields.filter(l => l.trim() && !l.trim().startsWith('@')).length}`);
