...
export const CreateStoryFormSchema = z.object({
  ...
  synopsis: z.string().min(50, "Synopsis must be at least 50 characters").max(2000, "Synopsis must be less than 2000 characters"),
  genre: z.array(z.enum(STORY_GENRES as unknown as [string, ...string[]])).min(1, "Select at least one genre"),
  ageRating: z.nativeEnum(AgeRating),
  contentWarnings: z.array(z.enum(CONTENT_WARNINGS as unknown as [string, ...string[]])),
  ratingExplanation: z.string().max(500).optional(),
  coverImageUrl: z.string().url().optional().or(z.literal("")),
  chapterContent: z.string().min(500, "First chapter must be at least 500 characters"),
});
...
export const UpdateStoryFormSchema = z.object({
  ...
  genre: z.array(z.enum(STORY_GENRES as unknown as [string, ...string[]])).optional(),
  ageRating: z.nativeEnum(AgeRating).optional(),
  contentWarnings: z.array(z.enum(CONTENT_WARNINGS as unknown as [string, ...string[]])).optional(),
