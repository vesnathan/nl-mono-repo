      ...
      {aiCreated && showAIBadge && (
        <Tooltip content="This story was created with AI assistance">
          <Chip
            size="sm"
            variant="solid"
            className="bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold px-3"
          >
            🤖 AI Generated
          </Chip>
        </Tooltip>
      ...
      {featured && showFeatured && (
        <Tooltip content="Featured story selected by our team">
          <Chip
            size="sm"
            variant="flat"
            className="bg-yellow-600 text-white px-3"
          >
            ⭐ Featured
          </Chip>
        </Tooltip>
      ...
      {displayGenres.map((genre) => (
        <Tooltip key={genre} content={`Genre: ${genre}`}>
          <Chip
            size="sm"
            variant="flat"
            className="bg-[#00457C] text-white px-3"
          >
            {genre}
          </Chip>
        </Tooltip>
