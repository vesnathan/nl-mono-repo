...
if (process.env.NODE_ENV === "development") {
  ...
  console.log(
    ...
    AMPLIFY_CONFIG.Auth?.Cognito?.userPoolId,
  );
  console.log(
    "Auth.Cognito.userPoolClientId:",
    AMPLIFY_CONFIG.Auth?.Cognito?.userPoolClientId,
  );
  console.log(
    "Auth.Cognito.identityPoolId:",
    AMPLIFY_CONFIG.Auth?.Cognito?.identityPoolId,
  );
...
export default function EnvTest() {
  return (
    <div style={{ padding: "20px", fontFamily: "monospace" }}>
      ...
      <ul>
        <li>
          USER_POOL_ID: {process.env.NEXT_PUBLIC_USER_POOL_ID || "NOT SET"}
        </li>
        <li>
          USER_POOL_CLIENT_ID:{" "}
          {process.env.NEXT_PUBLIC_USER_POOL_CLIENT_ID || "NOT SET"}
        </li>
        <li>
          IDENTITY_POOL_ID:{" "}
          {process.env.NEXT_PUBLIC_IDENTITY_POOL_ID || "NOT SET"}
        </li>
      ...
      <ul>
        <li>
          userPoolId: {AMPLIFY_CONFIG.Auth?.Cognito?.userPoolId || "NOT SET"}
        </li>
        <li>
          userPoolClientId:{" "}
          {AMPLIFY_CONFIG.Auth?.Cognito?.userPoolClientId || "NOT SET"}
        </li>
        <li>
          identityPoolId:{" "}
          {AMPLIFY_CONFIG.Auth?.Cognito?.identityPoolId || "NOT SET"}
        </li>
        <li>
          GraphQL endpoint: {AMPLIFY_CONFIG.API?.GraphQL?.endpoint || "NOT SET"}
        </li>
      </ul>
    </div>
  );
