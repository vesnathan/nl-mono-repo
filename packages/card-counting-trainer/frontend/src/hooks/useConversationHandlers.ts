...
export function useConversationHandlers({
  ...
  const handleConversationResponse = useCallback(
    ...
    (choiceIndex: number) => {
      ...
      debugLog(
        "conversations",
        `=== CONVERSATION RESPONSE (Choice ${choiceIndex}) ===`,
      );
      ...
      if (choice.suspicionChange !== 0) {
        debugLog(
          "conversations",
          `Pit boss suspicion: ${choice.suspicionChange > 0 ? "+" : ""}${choice.suspicionChange}`,
        );
      ...
      if (isPlayerCounting && currentDealer && !currentDealer.onYourSide) {
        ...
        if (choiceIndex === 0) {
          ...
          debugLog(
            "conversations",
            `Friendly response while counting → +2 dealer suspicion (${currentDealer.name})`,
          );
          ...
          debugLog(
            "conversations",
            `Neutral response while counting → +0.5 dealer suspicion (${currentDealer.name})`,
          );
          ...
          debugLog(
            "conversations",
            `Dismissive response while counting → +4 dealer suspicion (${currentDealer.name})`,
          );
        }

        if (dealerSuspicionChange > 0) {
          increaseDealerSuspicion(
            currentDealer,
            dealerSuspicionChange,
            setDealerSuspicion,
          );
        }
      } else if (
        ...
        debugLog(
          "conversations",
          `${currentDealer.name} is on your side - no dealer suspicion added`,
        );
  ...
  const handleConversationIgnore = useCallback(() => {
    ...
    if (currentDealer && !currentDealer.onYourSide) {
      if (isPlayerCounting) {
        ...
        debugLog(
          "conversations",
          `Ignored while counting → +6 dealer suspicion (${currentDealer.name})`,
        );
        ...
        debugLog(
          "conversations",
          `Ignored while not counting → +2 dealer suspicion (${currentDealer.name})`,
        );
      ...
      debugLog(
        "conversations",
        `${currentDealer.name} is on your side - ignoring doesn't raise dealer suspicion`,
      );
    }
