...
const S3_BUCKET =
  process.env.NEXT_PUBLIC_S3_BUCKET || "nlmonorepo-bjcct-frontend-dev";
...
function extractDialogueLines(): DialogueLine[] {
  ...
  if (!fs.existsSync(conversationsPath)) {
    console.warn(
      "Warning: conversations.ts not found, skipping dialogue extraction",
    );
    return lines;
  }
...
async function processLine(
  ...
  try {
    ...
    if (IS_PRODUCTION) {
      ...
      console.log(
        `[${index + 1}/${total}] Generating: ${text.substring(0, 50)}...`,
      );
      ...
      if (fs.existsSync(localPath)) {
        console.log(
          `[${index + 1}/${total}] Cached: ${text.substring(0, 50)}...`,
        );
        return;
      }
      console.log(
...
async function main() {
  ...
  console.log(
    `Environment: ${IS_PRODUCTION ? "Production (S3)" : "Development (Local)"}`,
  );
