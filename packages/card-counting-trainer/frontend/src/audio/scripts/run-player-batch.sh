...
# Use AI players manifest
MANIFEST="ai-players-manifest.json"

# Strip comments (in case there are any)
grep -v '^\s*/\*' "$MANIFEST" | grep -v '^\s*\*/' | sed 's|/\*[^*]*\*/||g' > ai-players-manifest-clean.json

jq -c '.items[]' ai-players-manifest-clean.json | while read -r ITEM; do
