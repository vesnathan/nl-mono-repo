...
export const DEALER_CHARACTERS: DealerCharacter[] = [
  ...
  },
  {
    ...
    backstory:
      "Harold has been dealing for 30 years and takes his job VERY seriously. He sees card counters as cheaters trying to steal from the casino, and he takes it personally. He's memorized every tell, every betting pattern, every sign of a skilled player. He deals fast to make counting harder and watches bet spreads like a hawk. If he suspects you're counting, he'll immediately call the pit boss over. He's gotten dozens of counters backed off over the years and he's proud of it.",
  },
  {
    id: "friendly-marcus",
    ...
    backstory:
      "Marcus is everyone's favorite dealer. He's a former bartender who loves chatting with players, telling jokes, and making sure everyone has a good time. He's not particularly observant when it comes to advantage play - he's too busy making people laugh. Even if he did notice someone counting, he probably wouldn't care. 'Hey, if you're smart enough to beat the house, more power to ya!' is his philosophy. He might deal a bit faster when the pit boss is watching just to look professional, but otherwise he's just here for the vibes.",
  },
  {
    id: "oblivious-frank",
    name: "Frank O'Brien",
...
export function getRandomDealer(excludeIds: string[] = []): DealerCharacter {
  ...
  if (availableDealers.length === 0) {
    return DEALER_CHARACTERS[
      Math.floor(Math.random() * DEALER_CHARACTERS.length)
    ];
  }
...
export function doesDealerNotice(
  ...
  const detectionScore =
    (baseSuspicion * 0.4 + betSpreadFactor * 0.4 + strategyFactor * 0.2) *
