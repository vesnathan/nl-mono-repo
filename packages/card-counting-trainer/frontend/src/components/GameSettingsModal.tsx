...
import { GameSettings, CountingSystem, TrainingMode, BlackjackPayout } from "@/types/gameSettings";
...
export default function GameSettingsModal({
  ...
  const loadPreset = (preset: "vegas" | "single" | "double" | "european" | "bad") => {
  ...
  const calculateHouseEdge = (): number => {
    ...
    else if (settings.blackjackPayout === BlackjackPayout.TWO_TO_ONE) edge -= 2.27;
    else if (settings.blackjackPayout === BlackjackPayout.EVEN_MONEY) edge += 2.27;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div
            style={{
              backgroundColor: houseEdge > 1 ? "rgba(244, 67, 54, 0.2)" : "rgba(76, 175, 80, 0.2)",
          ...
          >
            ...
            <div style={{ fontSize: "32px", fontWeight: "bold", color: houseEdge > 1 ? "#F44336" : "#4CAF50" }}>
              {houseEdge}%
            </div>
            {houseEdge > 1 && (
              <div style={{ fontSize: "12px", color: "#F44336", marginTop: "4px", fontStyle: "italic" }}>
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            <div style={{ marginBottom: "16px" }}>
              <label style={{ fontSize: "14px", color: "#AAA", display: "block", marginBottom: "8px" }}>
                Number of Decks
              </label>
              <div style={{ display: "flex", gap: "8px" }}>
                {([1, 2, 4, 6, 8] as const).map((num) => (
                  <button
                    key={num}
                    onClick={() => setSettings({ ...settings, numberOfDecks: num })}
                    style={{
                      backgroundColor: settings.numberOfDecks === num ? "#4A90E2" : "rgba(255, 255, 255, 0.1)",
                      color: settings.numberOfDecks === num ? "#FFF" : "#AAA",
                      border: "2px solid",
                      borderColor: settings.numberOfDecks === num ? "#FFF" : "rgba(255, 255, 255, 0.2)",
                      borderRadius: "8px",
                      padding: "10px 20px",
                      fontSize: "14px",
                      fontWeight: settings.numberOfDecks === num ? "bold" : "normal",
                      cursor: "pointer",
                      transition: "all 0.2s ease",
                      flex: 1,
                    }}
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, deckPenetration: parseInt(e.target.value) })}
                style={{
                  width: "100%",
                  accentColor: "#4A90E2",
                }}
            ...
            >
              <div>
                ...
                <div style={{ fontSize: "12px", color: "#AAA", marginTop: "4px" }}>
                  {settings.dealerHitsSoft17 ? "Dealer Hits (H17) - House Favored" : "Dealer Stands (S17) - Player Favored"}
                </div>
              </div>
              <button
                onClick={() => setSettings({ ...settings, dealerHitsSoft17: !settings.dealerHitsSoft17 })}
                style={{
                  backgroundColor: settings.dealerHitsSoft17 ? "#F44336" : "#4CAF50",
                  color: "#FFF",
                  border: "none",
                  borderRadius: "8px",
                  padding: "10px 20px",
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "8px" }}>
                {([
                  { value: BlackjackPayout.THREE_TO_TWO, label: "3:2", note: "(Standard)" },
                  { value: BlackjackPayout.SIX_TO_FIVE, label: "6:5", note: "(Bad!)" },
                  { value: BlackjackPayout.TWO_TO_ONE, label: "2:1", note: "" },
                  { value: BlackjackPayout.EVEN_MONEY, label: "1:1", note: "" },
                ]).map(({ value, label, note }) => (
                  <button
                    key={value}
                    onClick={() => setSettings({ ...settings, blackjackPayout: value })}
                    style={{
                      backgroundColor: settings.blackjackPayout === value ? "#4A90E2" : "rgba(255, 255, 255, 0.1)",
                      color: settings.blackjackPayout === value ? "#FFF" : (value === BlackjackPayout.SIX_TO_FIVE || value === BlackjackPayout.EVEN_MONEY) ? "#F44336" : "#AAA",
                      border: "2px solid",
                      borderColor: settings.blackjackPayout === value ? "#FFF" : "rgba(255, 255, 255, 0.2)",
                      borderRadius: "8px",
                      padding: "10px",
                      fontSize: "14px",
                      fontWeight: settings.blackjackPayout === value ? "bold" : "normal",
                      cursor: "pointer",
                      transition: "all 0.2s ease",
                    }}
                  >
          ...
          <div style={{ marginBottom: "24px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {(Object.keys(COUNTING_SYSTEMS) as Array<keyof typeof COUNTING_SYSTEMS>).map((system) => (
                <button
                  key={system}
                  onClick={() => setSettings({ ...settings, countingSystem: CountingSystem[system] })}
                  style={{
                    backgroundColor: settings.countingSystem === system ? "rgba(74, 144, 226, 0.3)" : "rgba(255, 255, 255, 0.05)",
                    color: "#FFF",
                    border: "2px solid",
                    borderColor: settings.countingSystem === system ? "#4A90E2" : "rgba(255, 255, 255, 0.1)",
                    borderRadius: "12px",
                    padding: "12px",
                    textAlign: "left",
                    cursor: "pointer",
          ...
          <div style={{ marginBottom: "32px" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {([
                ...
                <button
                  key={value}
                  onClick={() => setSettings({ ...settings, trainingMode: value })}
                  style={{
                    backgroundColor: settings.trainingMode === value ? "#4A90E2" : "rgba(255, 255, 255, 0.1)",
                    color: settings.trainingMode === value ? "#FFF" : "#AAA",
                    border: "2px solid",
                    borderColor: settings.trainingMode === value ? "#FFF" : "rgba(255, 255, 255, 0.2)",
                    borderRadius: "8px",
                    padding: "10px 16px",
                    fontSize: "13px",
                    fontWeight: settings.trainingMode === value ? "bold" : "normal",
                    cursor: "pointer",
                    transition: "all 0.2s ease",
                    flex: 1,
                  }}
