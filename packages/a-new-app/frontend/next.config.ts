...
const getDeploymentOutputs = () => {
  try {
    ...
    // The stack name is 'ANewApp' (matching StackType enum)
    const awseStack = outputs.stacks.ANewApp;
    if (!awseStack) {
      console.warn(
        "ANewApp stack outputs not found in deployment-outputs.json",
      );
    ...
    const envVars = {
      NEXT_PUBLIC_USER_POOL_ID: getValue("ANAUserPoolId"),
      NEXT_PUBLIC_USER_POOL_CLIENT_ID: getValue("ANAUserPoolClientId"),
      NEXT_PUBLIC_IDENTITY_POOL_ID: getValue("ANAIdentityPoolId"),
    ...
    if (missingVars.length > 0) {
      console.warn(
        "⚠️  ANewApp stack is deployed but missing outputs:",
        missingVars.join(", "),
      );
      console.warn(
