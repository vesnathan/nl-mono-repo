...
export function getDeploymentOutput<T extends keyof DeploymentOutputMap>(
  ...
  switch (outputType) {
    case "ANA": {
      const deploymentOutput: ANAOutput = {
      ...
      if (
        ...
        console.error(
          "Missing required environment variables for ANA configuration.",
        );
        // Depending on the desired behavior, you might want to throw an error
        // throw new Error('Missing required environment variables for ANA configuration.');
      }
