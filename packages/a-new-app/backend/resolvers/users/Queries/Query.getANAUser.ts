...
import { ANAUser, ClientType, GetANAUserQueryVariables } from "gqlTypes";
// Import ClientTypes constants from the single source of truth
import {
  ANA_CLIENT_TYPES,
  isValidANAClientType,
} from "../../../constants/ClientTypes";

// Use GetANAUserQueryVariables from shared frontend-generated types

// Define Output type for the resolver - it's ANAUser as per schema for a successful response
type Output = ANAUser;

// Define CTX for the response function context
// Args, Result, PrevResult, Source, Info
type CTX = Context<GetANAUserQueryVariables, object, object, object, Output>;

export function request(ctx: Context<GetANAUserQueryVariables>) {
  // It's good practice to cast args to the specific type
  const args = ctx.args as GetANAUserQueryVariables;
  const { userId } = args;
  ...
  // DynamoDB ANADataTable uses a single-table PK/SK schema.
...
export function response(ctx: CTX): Output {
  ...
  // Construct the final ANAUser object
  // Ensure all non-nullable fields of ANAUser are present.
  const resolvedUser: ANAUser = {
