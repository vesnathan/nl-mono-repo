...
import { ANAUserInput, ANAUser } from "gqlTypes";

// Define Input type for the resolver
type CreateANAUserMutationVariables = {
  input: ANAUserInput;
};

// Define Output type for the resolver - it's ANAUser as per schema
type Output = ANAUser;
...
export function request(ctx: Context<CreateANAUserMutationVariables>) {
...
export function response(ctx: CTX): Output {
  ...
  // We need to construct the ANAUser object that matches the GraphQL schema.
  ...
  // The clientType will be populated by Query.getANAUser based on Cognito groups
