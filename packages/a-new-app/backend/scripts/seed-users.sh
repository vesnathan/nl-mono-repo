...
echo -e "${BLUE}🌱 A New App User Seeding Script${NC}"
...
ls -al packages/a-new-app/backend/scripts

# Use yarn to run the seeding script with local dependencies
if command -v yarn &> /dev/null; then
    echo "[DEBUG] Running: yarn --cwd packages/a-new-app/backend run seed:users"
    if yarn --cwd packages/a-new-app/backend run seed:users; then
        echo "[DEBUG] yarn --cwd packages/a-new-app/backend run seed:users succeeded"
    else
