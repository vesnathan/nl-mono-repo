...
# Script to seed DynamoDB with A New App test users
set -e
...
echo -e "${BLUE}🌱 A New App DB Seeding Script${NC}"
echo ""
...
if [ -n "$TABLE_NAME" ]; then
    ...
    if command -v aws &> /dev/null && aws --version &> /dev/null; then
        TABLE_NAME=$(aws cloudformation list-exports \
            --region "$AWS_REGION" \
            --query "Exports[?Name=='ana-DataTableName-${STAGE}'].Value" \
            --output text 2>/dev/null || echo "nlmonorepo-ana-datatable-${STAGE}")
    else
        # AWS CLI not available, use default naming convention
        TABLE_NAME="nlmonorepo-ana-datatable-${STAGE}"
...
if command -v yarn &> /dev/null; then
    echo "[DEBUG] Running: yarn --cwd packages/a-new-app/backend run seed:db"
    if yarn --cwd packages/a-new-app/backend run seed:db; then
