...
const TABLE_NAME = process.env.TABLE_NAME || "nlmonorepo-ana-datatable-dev";
...
function createANAUser(superAdminUserId: string, userData: any): ANAUserDB {
  const userId = generateDeterministicUserId(userData.email);

  const user: ANAUserDB = {
    PK: `USER#${userId}`,
    SK: `PROFILE#${userId}`,
    userId,
...
async function insertUser(user: ANAUserDB): Promise<void> {
  try {
...
async function seedUsers() {
  try {
    ...
    for (const userData of SEED_DATA.users) {
      const user = createANAUser(superAdminUserId, userData);
      allItems.push(user);
    ...
    for (const item of allItems) {
      try {
        await insertUser(item as ANAUserDB);
        successCount++;
