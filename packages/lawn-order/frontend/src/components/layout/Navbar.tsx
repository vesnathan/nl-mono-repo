...
export function Navbar() {
  ...
  return (
    <>
      <nav className={`fixed w-full z-40 transition-all duration-300 ${scrolled ? 'bg-black/80' : 'bg-transparent'}`}>
        <div className="container mx-auto px-4">
          <div className={`flex md:justify-between justify-center items-center transition-all duration-300 ${scrolled ? 'py-2' : 'py-4'}`}>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-black/80 z-50 md:hidden transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className={`fixed right-0 top-0 h-full w-64 bg-brand-dark transform transition-transform duration-300 z-50 ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
