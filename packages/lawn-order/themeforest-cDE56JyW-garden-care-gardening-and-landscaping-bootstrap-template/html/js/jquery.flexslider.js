  ...
  e.flexslider = function (t, n) {
    ...
    p = {
      init: function () {
        ...
        if (i.controlsContainer !== "")
          r.controlsContainer =
            e(i.controlsContainer).length > 0 && e(i.controlsContainer);
        if (i.manualControls !== "")
          r.manualControls =
            e(i.manualControls).length > 0 && e(i.manualControls);
        if (i.randomize) {
        ...
        setup: function () {
          ...
          r.slides.click(function (t) {
            ...
            var n = e(this),
              s = n.index();
            if (
              !e(i.asNavFor).data("flexslider").animating &&
              !n.hasClass("active")
            ) {
        ...
        setupPaging: function () {
          ...
          if (r.pagingCount > 1) {
            for (var f = 0; f < r.pagingCount; f++) {
              a =
                i.controlNav === "thumbnails"
                  ? '<img src="' + r.slides.eq(f).attr("data-thumb") + '"/>'
                  : "<a>" + n + "</a>";
      ...
      touch: function () {
        function d(u) {
          if (r.animating) {
            ...
            h = Number(new Date());
            s =
              l && f && r.animatingTo === r.last
                ? 0
                : l && f
                  ? r.limit - (r.itemW + i.itemMargin) * r.move * r.animatingTo
                  : l && r.currentSlide === r.last
                    ? r.limit
                    : l
                      ? (r.itemW + i.itemMargin) * r.move * r.currentSlide
                      : f
        ...
        function v(t) {
          ...
          p = a
            ? Math.abs(u) < Math.abs(t.touches[0].pageX - n)
        ...
        function m(a) {
          ...
          if (r.animatingTo === r.currentSlide && !p && !(u === null)) {
            ...
            if (
              r.canAdvance(d) &&
              ((Number(new Date()) - h < 550 && Math.abs(l) > 50) ||
                Math.abs(l) > o / 2)
            ) {
        ...
        var n = e ? e : (r.itemW + i.itemMargin) * r.move * r.animatingTo,
          s = (function () {
            if (l) {
              return t === "setTouch"
                ? e
                : f && r.animatingTo === r.last
                  ? 0
                  : f
                    ? r.limit -
                      (r.itemW + i.itemMargin) * r.move * r.animatingTo
                    : r.animatingTo === r.last
                      ? r.limit
                      : n;
            } else {
    ...
    r.setup = function (t) {
      if (!c) {
        var n, u;
        if (t === "init") {
          r.viewport = e('<div class="' + s + 'viewport"></div>')
        ...
        if (i.animationLoop && !l) {
          ...
          r.container
            .append(r.slides.first().clone().addClass("clone"))
          ...
          if (!o) {
            ...
            r.slides
              .css({
                ...
                display: "block",
                webkitTransition:
                  "opacity " + i.animationSpeed / 1e3 + "s ease",
              })
              .eq(r.currentSlide)
              .css({ opacity: 1 });
          }
