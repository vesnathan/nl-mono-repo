# App Builder Studio - Email Setup
...
### Lambda Function
- **Location**: `packages/app-builder-studio/backend/lambda/sendContactEmail.ts`
...
- `FROM_EMAIL`: `noreply@appbuilderstudio.com`
- `TO_EMAIL`: `hello@appbuilderstudio.com`
...
```bash
# Verify FROM email
aws ses verify-email-identity --email-address noreply@appbuilderstudio.com --region ap-southeast-2

# Verify TO email
aws ses verify-email-identity --email-address hello@appbuilderstudio.com --region ap-southeast-2
```
...
3. Check CloudWatch Logs: `/aws/lambda/nlmonorepo-appbuilderstudio-send-contact-email-dev`
...
View logs:
```bash
aws logs tail /aws/lambda/nlmonorepo-appbuilderstudio-send-contact-email-dev --follow --region ap-southeast-2
```
