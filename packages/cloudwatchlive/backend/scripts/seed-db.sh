...
echo -e "${BLUE}🌱 CWL DB Seeding Script${NC}"
...
# Get table name from environment or prefer deployment-outputs.json, then CloudFormation exports, then default
...
if command -v yarn &> /dev/null; then
    echo "[DEBUG] Running: yarn --cwd packages/cloudwatchlive/backend run seed:db"
    if yarn --cwd packages/cloudwatchlive/backend run seed:db; then
        echo "[DEBUG] yarn --cwd packages/cloudwatchlive/backend run seed:db succeeded"
    else
        echo "[DEBUG] yarn --cwd failed, attempting yarn workspace fallback"
        if yarn workspace cwlbackend run seed:db; then
            echo "[DEBUG] yarn workspace cwlbackend run seed:db succeeded"
        else
